                ...
                <Tabs value={mode} onValueChange={(v: string) => setMode(v as "manual" | "file")}>
                    ...
                    <TabsContent value="manual" className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex justify-between text-xs text-muted-foreground">
                                <span>
                                    {jobDescription.length > 15000 ? (
                                        <span className="text-red-500">{jobDescription.length} characters (max 15000)</span>
                                    ) : warnings.jobDescription ? (
                                        <span className="text-yellow-500">
                                            {jobDescription.length} characters - {warnings.jobDescription}
                                        </span>
                                    ) : (
                            ...
                            {touchedFields.jobDescription && errors.jobDescription && <p className="text-sm text-red-500">{errors.jobDescription}</p>}
                        </div>

                        <div className="space-y-3">
                            <div className="flex items-center space-x-2">
                                <Checkbox id="slice-projects" checked={sliceProjects} onCheckedChange={(checked: boolean) => setSliceProjects(checked)} />
                                <label htmlFor="slice-projects" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 cursor-pointer flex items-center gap-2">
                    ...
                    <TabsContent value="file" className="space-y-4">
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center space-x-2">
                                <Checkbox id="slice-projects-batch" checked={sliceProjects} onCheckedChange={(checked: boolean) => setSliceProjects(checked)} />
                                <label htmlFor="slice-projects-batch" className="text-sm font-medium leading-none cursor-pointer flex items-center gap-2">
                                    <Scissors className="h-4 w-4 text-primary" /> Apply project slicing during batch
                                </label>
                            </div>
                        </div>
